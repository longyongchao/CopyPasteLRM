...
def analyze_hotpotqa():
    ...
    # 按类型和难度分组存储数据
    stats = defaultdict(lambda: {"answer_lengths": [], "context_lengths": [], "supporting_fact_counts": [], "supporting_title_counts": [], "supporting_sentence_lengths": [], "context_title_counts": []})
...
def create_bar_charts(stats):
    ...
    # 为每个指标准备数据
    metrics = {"Answer Length": {"mean": [], "std": []}, "Context Length": {"mean": [], "std": []}, "Supporting Facts Count": {"mean": [], "std": []}, "Unique Title Count": {"mean": [], "std": []}, "Supporting Sentence Length": {"mean": [], "std": []}, "Context Title Count": {"mean": [], "std": []}}
    ...
    for i, metric_name in enumerate(metric_names):
        ...
        # 绘制柱状图（带误差线）
        bars1 = ax.bar(x - width / 2, bridge_means, width, label="Bridge", alpha=0.8, color=colors["Bridge"], yerr=bridge_stds, capsize=5)
        bars2 = ax.bar(x + width / 2, comp_means, width, label="Comparison", alpha=0.8, color=colors["Comparison"], yerr=comp_stds, capsize=5)
